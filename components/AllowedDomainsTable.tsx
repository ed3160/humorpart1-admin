...
export default function AllowedDomainsTable({ filter, userId }: { navigateTo: (section: string, filter?: NavFilter) => void; filter: NavFilter | null; userId: string }) {
  ...
  const startEdit = (row: AllowedDomain) => {
    setEditId(row.id);
    setEditDomain(row.apex_domain);
  };
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-neutral-900 mb-4">Allowed Signup Domains</h2>

      {/* Create form */}
      <div className="bg-white rounded-lg border border-neutral-200 p-4 mb-4">
        <h3 className="text-sm font-semibold text-neutral-700 mb-2">Add Domain</h3>
        <div className="flex items-center gap-2">
          <input type="text" placeholder="e.g. columbia.edu" value={newDomain} onChange={(e) => setNewDomain(e.target.value)} className="border border-neutral-300 rounded-lg px-3 py-2 text-sm flex-1 focus:outline-none focus:ring-2 focus:ring-neutral-400" />
          <button onClick={handleCreate} disabled={creating || !newDomain.trim()} className="bg-neutral-900 text-white px-3 py-2 rounded-lg text-sm font-medium hover:bg-neutral-800 disabled:opacity-40 cursor-pointer disabled:cursor-default">Add</button>
        </div>
      </div>

      <span className="text-xs text-neutral-400 mb-4 block">{total} domains</span>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-neutral-50 border-b border-neutral-200">
              ...
              <th className="text-left px-3 py-2 font-medium text-neutral-600 cursor-pointer hover:text-neutral-900 select-none whitespace-nowrap" onClick={() => toggleSort("apex_domain")}>Domain{sortIcon("apex_domain")}</th>
          ...
          <tbody className="divide-y divide-neutral-100">
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-3 py-4 text-neutral-500">No domains found.</td></tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id} className="hover:bg-neutral-50">
                  ...
                  <td className="px-3 py-2">
                    {editId === r.id ? (
                      <input type="text" value={editDomain} onChange={(e) => setEditDomain(e.target.value)} className="border border-neutral-300 rounded px-2 py-1 text-sm w-full" />
                    ) : (
                      <span className="text-neutral-700 font-medium">{r.apex_domain}</span>
                    )}
                  </td>
